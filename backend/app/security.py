...
class InputSanitizer:
    ...
    def clean(self, text: str) -> str:
        """Strip delimiter sequences that could confuse the LLM."""
...
class PIIDetector:
    ...
    def detect(self, text: str) -> dict[str, list[str]]:
        """Return a mapping of PII type → list of matched strings."""
        return {
...
class OutputValidator:
    ...
    def validate(self, output: str) -> tuple[str, list[str]]:
        """Return (cleaned_output, warnings). Masks PII and blocks harmful content."""
        warnings: list[str] = []
...
class SecurityPipeline:
    ...
    @traceable(name="security_check_input", hide_inputs=True)
    def check_input(self, text: str) -> tuple[bool, str, list[str]]:
        """
        Run input through injection detection → delimiter cleaning → PII masking.
    ...
    @traceable(name="security_check_output", hide_inputs=True)
